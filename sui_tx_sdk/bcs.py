...
class Deserializer:
    ...
    def map(
        self,
        key_decoder: typing.Callable[[Deserializer], T],
        value_decoder: typing.Callable[[Deserializer], U],
    ) -> typing.Dict[T, U]:
        length = self.uleb128()
        values = {}
    ...
    def sequence(
        self,
        value_decoder: typing.Callable[[Deserializer], T],
    ) -> typing.List[T]:
        length = self.uleb128()
        values = []
        while len(values) < length:
...
class Serializer:
    ...
    def map(
        self,
        values: typing.Dict[T, U],
        key_encoder: typing.Callable[[Serializer, T], None],
        value_encoder: typing.Callable[[Serializer, U], None],
    ):
        encoded_values = []
        for (key, value) in values.items():
            encoded_values.append(
                (self._encoding(key, key_encoder), self._encoding(value, value_encoder))
            )
    ...
    def sequence_serializer(
        value_encoder: typing.Callable[[Serializer, T], None],
    ):
    ...
    def sequence(
        ...
        for value in values:
            self.fixed_bytes(self._encoding(value, value_encoder))

    def str(self, value: str):
        self.bytes(value.encode())

    def struct(self, value: T):
    ...
    def _encoding(
        value: T,
        encoder: typing.Callable[[Serializer, T], None],
    ) -> bytes:
        ser = Serializer()
        encoder(ser, value)
        return ser.output()
